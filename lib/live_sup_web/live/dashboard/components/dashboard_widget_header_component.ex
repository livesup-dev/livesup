  ...
  def render(assigns) do
    ...
    <div class="justify-between flex flex-row space-x-4 border-b dark:border-primary-darker px-4 py-2">
      <div class="flex items-center basis-4/5 flex-wrap">
        ...
        <%= link @project.name, class: "text-blue-500 after:mx-4 after:content-['>'] dark:after:text-white hidden md:block", to: Routes.dashboard_path(LiveSupWeb.Endpoint, :index, @project.id) %>
        ...
        <%= link @dashboard.name, class: "text-blue-500 after:mx-4 after:content-['>'] dark:after:text-white", to: Routes.dashboard_path(LiveSupWeb.Endpoint, :show, @dashboard.id) %>
        <span>Widgets</span>
      </div>
    </div>
    """
