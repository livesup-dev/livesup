  ...
  def render(assigns) do
    ...
    <div class="justify-between flex flex-row space-x-4 border-b dark:border-primary-darker px-4 py-2">
      <div class="flex items-center basis-4/5 flex-wrap">
        ...
        <%= link "Teams", class: "text-blue-500 after:mx-4  after:content-['>'] dark:after:text-white", to: Routes.team_list_path(LiveSupWeb.Endpoint, :index) %>
        <span><%= @team.name %></span>
      </div>
      <div class="flex justify-end basis-1/5 text-right items-center">
        ...
        <% end %>
        <%= AddButtonComponent.render(%{path: Routes.members_path(LiveSupWeb.Endpoint, :add, @team.id)}) %>
      </div>
    </div>
    """
  end
