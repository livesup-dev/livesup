            ...
            <%= if Enum.any?(widget_data.data) do %>
              ...
              <div class="divide-y divide-gray-100 dark:divide-gray-500 ">
                ...
                <div class="gap-4 py-2 flex">
                  <a class="text-black dark:text-primary hover:underline break-all basis-9/12" target="_blank" href={"#{widget_data.public_settings["domain"]}/browse/#{issue[:key]}"}}><%= issue[:summary] %></a>
                  <span class="justify-center text-center basis-2/12"><%= issue[:status] %></span>
                  <span class="justify-center text-center basis-1/12"><%= LiveSup.Helpers.DateHelper.from_now(issue[:created_at], :short) %></span>
                </div>
                <% end %>
              </div>
            <% else %>
