        ...
        <div class="flex-auto divide-y divide-gray-100 dark:divide-dark p-2">
          <%= for {entry, counter} <- Enum.with_index(widget_data.data) do %>
            <div class="flex pt-2 pb-2">
              ...
              <% else %>
                <p class="basis-3/4"><a target="_blank" href={entry[:url]} target="blank" class="hover:underline"><%= entry[:service_name] %></a><br><a target="_blank" href={entry[:url]}><span class="text-blue-500 text-sm text-stone-400">Last incident: <%= entry[:created_at_ago] %></span></a></p>
                <p class="basis-1/6 pt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 m-auto" fill="none" viewBox="0 0 24 24" stroke="#21D3EE">
