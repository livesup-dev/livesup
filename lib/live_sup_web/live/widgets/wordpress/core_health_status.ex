  ...
  def render_widget(assigns) do
    ...
    <.live_component module={SmartRenderComponent} id="core-health-status" let={widget_data} widget_data={@widget_data}>
      <!-- WordPress Core Health -->
        ...
        <!-- Widget Content -->
        <div class="flex-1 grid grid-cols-1 divide-y divide-gray-100 dark:divide-dark text-sm p-2">
          <%= for {key, value} <- widget_data.data[:wp_core] do %>
            <p class="flex font-semibold justify-between py-2"><span><%= value[:label] %></span><span class="text-xs"><%= value[:value] %></span></p>
          <% end %>
        </div>
        <!-- /Widget Content -->
      <!-- /WordPress Core Health -->
