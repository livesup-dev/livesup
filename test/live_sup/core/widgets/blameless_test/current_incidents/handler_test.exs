defmodule LiveSup.Test.Core.Widgets.Blameless.CurrentIncidents.HandlerTest do
  ...
  describe "Managing blameless current incidents" do
    @describetag :widget
    @describetag :blameless_current_incidents_widget
    @describetag :blameless_current_incidents_widget_handler
    ...
    test "getting current incidents" do
      with_mock BlamelessDatasource,
        get_current_incidents: fn _args -> {:ok, @response} end do
        incidents =
          %{
            "client_id" => "xxx",
            "client_secret" => "xxx",
